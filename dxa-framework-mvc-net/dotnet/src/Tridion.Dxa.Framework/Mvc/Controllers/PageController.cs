...
using System;
using Tridion.Dxa.Framework.Mvc;
...
namespace Sdl.Web.Mvc.Controllers
{
    ...
    {
        public PageController(IContentProvider contentProvider) : base()
        {
        ...
        public virtual ActionResult Page(string pageUrl)
        {
            ...
            string absoluteUrlPath = Request.Path.Value;//Url.AbsolutePath;
        ...
        public new virtual ActionResult NotFound()
        {
            using (new Tracer())
            {
                string notFoundPageUrl = WebRequestContext.Current.Localization.Path + "/error-404"; // TODO TSI-775: No need to prefix with WebRequestContext.Current.Localization.Path here (?)

                PageModel pageModel;
                try
                {
                    pageModel = ContentProvider.GetPageModel(notFoundPageUrl, WebRequestContext.Current.Localization);
                }
                catch (DxaItemNotFoundException ex)
                {
                    Log.Error(ex);

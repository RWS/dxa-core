        ...
        public T GetOrAdd<T>(string key, string region, Func<T> addFunction, IEnumerable<string> dependencies = null)
        {
            ...
            try
            {
                ...
                if (usedBy == 0 || usedBy == threadId)
                    return CreateCacheValue(hash, key, region, addFunction, dependencies);
                ...
                return _slots[hash] == threadId ? CreateCacheValue(hash, key, region, addFunction, dependencies) : addFunction();
            }
            finally
            {

...
namespace Sdl.Web.Modules.DynamicDocumentation.Providers
{
    public class TocProvider
    {
        ...
        public TocProvider(IApiClientFactory apiClientFactory)
        {
            ...
            int descendantLevels)
        {
            return SiteConfiguration.CacheProvider.GetOrAdd(
                $"toc-{localization.Id}-{sitemapItemId}-{includeAncestors}-{WebRequestContext.Current.CacheKeySalt}", CacheRegion.Toc,
                () =>
                {
                    new PublicationProvider(_apiClientFactory).CheckPublicationOnline(int.Parse(localization.Id));
        ...
        public SitemapItem SiteMap(Common.Configuration.Localization localization)
        {
            return SiteConfiguration.CacheProvider.GetOrAdd(
                ...
                () =>
                {
                    ...
                    {
                        var pubs = new PublicationProvider(_apiClientFactory).PublicationList;

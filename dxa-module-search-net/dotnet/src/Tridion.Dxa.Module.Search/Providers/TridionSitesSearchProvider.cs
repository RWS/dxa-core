...
namespace Sdl.Web.Modules.Search.Providers
{
    public class TridionSitesSearchProvider : OpenSearchProvider
    {
        ...
        public TridionSitesSearchProvider(IApiClientFactory apiClientFactory) : base(apiClientFactory) // Pass the apiClientFactory to the base constructor
        {
            try
            {
                _apiClientFactory = apiClientFactory;
                _separator = DEFAULT_SEPARATOR; //WebConfigurationManager.AppSettings["iq-field-separator"] ?? 
                _defaultLanguage = DEFAULT_LANGUAGE; //WebConfigurationManager.AppSettings["iq-default-language"] ?? 
                _defaultContentFieldName = DEFAULT_CONTENT_FIELD_NAME; //WebConfigurationManager.AppSettings["iq-default-content-field"] ?? 
                _defaultDynamicFieldName = DEFAULT_DYNAMIC_FIELD_NAME; //WebConfigurationManager.AppSettings["iq-default-dynamic-field"] ?? 
                ...
                _namespace = DEFAULT_NAMESPACE; //WebConfigurationManager.AppSettings["iq-namespace"]?? 
        ...
        protected override SearchResultSet ExecuteQuery(NameValueCollection parameters)
        {
            using (new Tracer(parameters))
            {
                ...
                var lang = GetLanguage(WebRequestContext.Current.Localization.Culture);
                ICriteria criteria = null;
                
                var queryString = GetSearchQueryString(parameters["q"]);
                var pubId = WebRequestContext.Current.Localization.Id;

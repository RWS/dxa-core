...
namespace Tridion.Dxa.Api.Client
{
    ...
    public static class GraphQLRequests
    {
        ...
        public static IGraphQLRequest Topic(int publicationId, int pageId) =>
                new QueryBuilder().WithQueryResource("IshTopicById", true)
                    .WithPublicationId(publicationId)
                    .WithPageId(pageId)
                    .Build();

        public static IGraphQLRequest Topic(string url) =>
                new QueryBuilder().WithQueryResource("IshTopicByUrl", true)
                    .WithUrl(url)
                    .Build();

        public static IGraphQLRequest Topics(InputIshItemFilter filter, InputSortParam sort,
            IPagination pagination, string customMetaFilter, IshTopicRenderOptions ishTopicRenderingOption)
        {
            QueryBuilder builder = new QueryBuilder().WithQueryResource("IshTopics", true);

            return builder
                .WithPagination(pagination)
                .WithCustomMetaFilter(customMetaFilter)
                .WithInputIshItemFilter(filter)
                .WithInputSortParam(sort)
                .WithVariable("isTopicRenderingOption", Enum.GetName(typeof(IshTopicRenderOptions), ishTopicRenderingOption))
                .Build();
        }

...
using Tridion.Dxa.Api.Client.Exceptions;
...
namespace Tridion.Dxa.Api.Client
{
    ...
    public class ApiClient : IGraphQLClient, IApiClient, IApiClientAsync
    {
        ...
        public IshTopicConnection GetTopics(InputIshItemFilter filter, InputSortParam sort, IPagination pagination,
            string customMetaFilter, IshTopicRenderOptions ishTopicRenderingOption)
        {
            try
            {
                var response = _client.Execute<ContentQuery>(GraphQLRequests.Topics(filter, sort, pagination,
                    customMetaFilter, ishTopicRenderingOption));
                return response.TypedResponseData.IshTopics;
            }
            catch (RuntimeBinderException e)
            {
                throw new ApiException(
                    $"Failed to get search results (GetTopics:{filter}{sort}{pagination}{customMetaFilter}{ishTopicRenderingOption})", e);
            }
        }

        public IshGenericTopic GetTopic(int publicationId, int topicItemId)
        {
            try
            {
                var response = _client.Execute<ContentQuery>(GraphQLRequests.Topic(publicationId, topicItemId));
                return response.TypedResponseData.IshTopic;
            }
            catch (RuntimeBinderException e)
            {
                throw new ApiException(
                    $"Failed to get search results (GetTopic:{publicationId} {topicItemId})", e);
            }
        }

        public IshGenericTopic GetTopic(string url)
        {
            try
            {
                var response = _client.Execute<ContentQuery>(GraphQLRequests.Topic(url));
                return response.TypedResponseData.IshTopic;
            }
            catch (RuntimeBinderException e)
            {
                throw new ApiException(
                    $"Failed to get search results (GetTopic:{url})", e);
            }
        }
